"""
DRR submodule for layer-wise normalized L0L2 regularized training.
...
# Function Hierarchy
- `DRR_train_FT!`: Full training with fine-tuning mask (master function)
- `DRR_train!`: Training without masking (special case of DRR_train_FT!)

# Exports
## Training Functions
- `DRR_train_FT!`: Layer-normalized L0L2 training with fine-tuning mask
- `DRR_train!`: Layer-normalized L0L2 training without masking
- `DRR_regularizing_terms`: Compute layer-normalized regularization terms
...
"""
function DRR_train_FT!(loss::Function, model::Union{Chain, Flux.Chain}, mask::Vector, data::Vector{<:Tuple}, opt, args)
    ...
    for d in data
        ...
        regularizing_terms = DRR_regularizing_terms(all_parameters, args, (Alpha, Rho, Two))
...
- `opt`: Flux optimizer
"""
function DRR_train_FT!(loss::Function, model::Union{Chain, Flux.Chain}, mask::Vector, data::Vector{<:Tuple}, opt)
    args = TrainArgs()
    DRR_train_FT!(loss, model, mask, data, opt, args)
end
...
"""
function DRR_train!(loss::Function, model::Union{Chain, Flux.Chain}, data::Vector{<:Tuple}, opt; device=gpu)
    args = TrainArgs()
    args.dev = device
    DRR_train!(loss, model, data, opt, args)
end
...
"""
function DRR_train!(loss::Function, model::Union{Chain, Flux.Chain}, data::Vector{<:Tuple}, opt, args)
    device = args.dev
...
"""
function DRR_regularizing_terms(all_parameters, args)
    ...
    return DRR_regularizing_terms(all_parameters, args, (Alpha, Rho, Two))
end
...
2ρw + αβ·sign(w)·exp(-β|w|) with layer-wise normalized α and ρ
"""
function DRR_regularizing_terms(all_parameters, args, constants::Tuple)
