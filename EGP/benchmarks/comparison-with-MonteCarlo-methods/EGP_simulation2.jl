...
n, p, p_sub, noise_factor, lambda_value = (200, 2000, 30, 0.75f0, 15f0)
...
for optimizerName in optimizerNames
    ...
    yval = Xval[:,1:p_sub] * beta_initial  .+ noise_factor .* noise_vector_val
    ...
    for (name, df_name) in names_df
        save_path = results_path * "/EGP_2_" * date_string *  nt_to_string(setting) * name * ".csv"
